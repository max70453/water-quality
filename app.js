...
function generateData() {
    ...
    waterData = regions.map(region => {
        const baseQuality = Math.random() > 0.3 ? Math.floor(Math.random() * 2) + 1 : Math.floor(Math.random() * 3) + 1;
        return {
            id: region.id,
            name: region.name,
            lat: region.lat,
...
function initCharts() {
    ...
    charts.metrics = new Chart(document.getElementById('metricsChart'), {
        type: 'bar',
        data: {
            labels: ['pH', 'Жёсткость', 'Хлор'],
            datasets: [{
                label: 'Среднее значение',
                data: avgValues,
                backgroundColor: ['#0ea5e9', '#84cc16', '#f97316'],
                borderRadius: 8
            }]
        },
        options: {
...
function populateSelectors() {
    ...
    document.getElementById('compareRegion1').addEventListener('change', updateComparison);
    document.getElementById('compareRegion2').addEventListener('change', updateComparison);
}
...
function updateDynamicsCharts() {
    ...
    ['ph', 'hardness', 'chlorine', 'qualityIndex'].forEach(key => {
    ...
    charts.qualityIndex = new Chart(document.getElementById('qualityIndexChart'), {
        ...
        data: {
            labels: labels,
            datasets: [{ label: 'Индекс', data: generateTrend(region.quality, 1), borderColor: '#7b2ff7', tension: 0.4, fill: true, backgroundColor: 'rgba(123,47,247,0.1)' }]
        },
...
function updateComparison() {
    const r1Id = document.getElementById('compareRegion1').value;
    const r2Id = document.getElementById('compareRegion2').value;

    const r1 = waterData.find(d => d.id == r1Id);
    const r2 = waterData.find(d => d.id == r2Id);

    if (r1) {
        document.getElementById('region1Info').innerHTML = renderRegionInfo(r1);
    }
    if (r2) {
        document.getElementById('region2Info').innerHTML = renderRegionInfo(r2);
    }
    ...
    if (r1 && r2) {
        ...
        charts.compare = new Chart(document.getElementById('compareChart'), {
            type: 'radar',
            data: {
                labels: ['pH', 'Жёсткость', 'Хлор', 'Качество'],
                datasets: [
                    { label: r1.name, data: [r1.ph / 14 * 100, r1.hardness / 20 * 100, r1.chlorine / 2 * 100, (6 - r1.quality) / 5 * 100], borderColor: '#0ea5e9', backgroundColor: 'rgba(14, 165, 233, 0.2)' },
                    { label: r2.name, data: [r2.ph / 14 * 100, r2.hardness / 20 * 100, r2.chlorine / 2 * 100, (6 - r2.quality) / 5 * 100], borderColor: '#f97316', backgroundColor: 'rgba(249,115,22,0.2)' }
                ]
            },
            options: {
                responsive: true,
                maintainAspectRatio: false,
                scales: {
                    r: { grid: { color: 'rgba(0,0,0,0.1)' }, ticks: { color: '#64748b', backdropColor: 'transparent' }, pointLabels: { color: '#1e293b' } }
                },
...
function exportData() {
    const csv = 'Регион,pH,Жёсткость,Хлор,Качество\n' + 
        waterData.map(d => `${d.name},${d.ph},${d.hardness},${d.chlorine},${qualityLabels[d.quality]}`).join('\n');
